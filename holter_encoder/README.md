...
- 교란 기준선은 네 종류다. `demo`(나이·성별·HR), `demo_nl`(나이 10세 구간 one-hot),
  `meta`(촬영 조건), `report`(벤더 리포트의 이소성·빈맥 지표), `encage`(인코더가 예측한 나이).
  인코더는 이 모두 대비로 해석한다. TOF 는 나이만으로 AUROC 0.91 이 나온다.
- 기준선과의 비교는 **짝지은 부트스트랩**(`[짝지은 차이]` 블록)으로 본다. 독립 CI 두 개가
  겹치는지 보는 것은 검정이 아니다 — PSVT enc 0.696 과 report 0.591 은 CI 가 겹치지만
  짝지은 차이는 +0.105 [+0.036-+0.175], P(Δ>0)=0.998 이다.
- 2026-09-22 기준 CV 결과: PSVT 는 모든 기준선 대비 P≥0.998, TOF 는 P=1.000,
  LongQT 는 report 대비 P=0.979 (양성 환자 24명, 경계선).
- LongQT test 양성 환자는 24명이다. 환자 단위 집계와 환자 부트스트랩 CI 로 보고한다.
- 인코더의 나이 예측은 MAE 5.3세 · r 0.23 으로 약하다. 소아 심전도는 나이에 따라
  HR·QT·QRS 가 크게 변하므로, 사전학습이 전역 속성을 담도록 강제하지 못한다는 신호다.
