...
CSV_PATH    = "/home/coder/workspace/data/remove_silent_bandpass_notch/h5_metadata_summary.csv"

# BASE_DIR: 원본 HDF5 데이터가 위치한 루트
BASE_DIR    = "/home/coder/workspace/data/remove_silent_bandpass_notch"

# RESULT_ROOT: 이번 실행 결과를 저장할 디렉토리 (타임스탬프 포함)
RESULT_ROOT = f"/home/coder/workspace/data/tykim/results0114/{datetime.now().strftime('%Y%m%d_%H%M%S')}"

# SPLIT_DIR: patient-wise split 결과 CSV들이 있는 디렉토리
# - 파일명 규칙: split_seed{00}.csv
SPLIT_DIR   = "/home/coder/workspace/data/remove_silent_bandpass_notch/seed_splits"  # split_seed{00}.csv

# SPLIT_SEEDS: 어떤 split 파일들을 돌릴지 결정
SPLIT_SEEDS = [98]

# CACHE_DIR: fixed bag(=record별 selected_keys) 캐시를 저장할 디렉토리
CACHE_DIR   = "/home/coder/workspace/data/tykim/seed_caches0114"   # 캐시 보관소
...
def scan_record(fp, label, seg_per_record, method, radius, fill, segment_seed, upsample_index=0):
    ...
    seed = _stable_seed(
        SEED, segment_seed, base, method, radius, seg_per_record, fill, int(upsample_index)
    )
    rng = random.Random(seed)

    with h5py.File(fp, "r") as f:
        segs = f.get("ECG/segments")
        if segs is None:
            return None

        raw_keys = list(segs.keys())
        if not raw_keys:
            return None

        # key 정렬: seg_000123 -> 123 기준
        idx_map = {_parse_idx_local(k): k for k in raw_keys}
        all_keys = [idx_map[i] for i in sorted(idx_map)]

        # S key 추출: beat_annotation/symbol 에 "S"
        s_keys_all = []
        for k in all_keys:
            g = segs[k]
            if "beat_annotation" in g and "symbol" in g["beat_annotation"]:
                ds = g["beat_annotation"]["symbol"]
                try:
                    arr = ds.asstr()[...]
                except Exception:
                    arr = ds[...]

                # arr가 bytes일 수도 있어서 문자열로 통일
                # (pipeline 코드가 문자열 "S"를 쓰므로 "S"로 비교)
                if isinstance(arr, np.ndarray) and arr.dtype.kind in ("S", "O"):
                    arr = np.array([x.decode() if isinstance(x, (bytes, np.bytes_)) else str(x) for x in arr])
...
class MILBagDatasetFixed(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(fp, "r") as f:
            segs = f.get("ECG/segments")
            if segs is None:
                bag_tensor = torch.zeros((0, len(SELECTED_LEADS), SEGMENT_LENGTH), dtype=torch.float32)
            ...
            for k in keys:
                ...
                sig_grp = g["signal"]  # group
                lead_names = list(sig_grp.keys())
                if not lead_names:
                    continue

                ref_key = lead_names[0]
                ref_arr = sig_grp[ref_key][:]

                # SELECTED_LEADS 순서대로 채널 구성, 없으면 0으로 대체
                leads = SELECTED_LEADS if SELECTED_LEADS else lead_names
                Xs = []
                for ld in leads:
                    arr = sig_grp[ld][:] if ld in sig_grp else np.zeros_like(ref_arr)
                    Xs.append(arr)

                x = np.asarray(Xs, dtype=np.float32)  # (C, L)

                # 길이 보정(pad/crop) -> (C, SEGMENT_LENGTH)
                if x.shape[1] < SEGMENT_LENGTH:
                    x = np.pad(x, ((0, 0), (0, SEGMENT_LENGTH - x.shape[1])))
                else:
                    x = x[:, :SEGMENT_LENGTH]

                bag.append(torch.tensor(x, dtype=torch.float32))
