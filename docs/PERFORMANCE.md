...
| 4차 수정 후 | **70,500 seg/s** | **100%** | `span_mask` 의 GPU 동기화 | 코드 검토 + 규모별 시간 측정 |
...
연속 구간 형태·`valid` 구간 보존·재현성 확인.

서버 결과: **6,661 → 70,500 seg/s (10.6배), GPU util 100%.** 처음 551 seg/s 대비 128배다.
30,000 step(2억 4,580만 세그먼트)이 약 1시간이 됐다.

전체 과정에서 가장 큰 한 방이 이것이었다. 앞선 세 번은 모두 "느린 것은 IO" 라는 가정
아래 IO 를 고쳤고 실제로 조금씩 나아졌지만, **정작 범인은 GPU 를 세우는 한 줄**이었다.
`bench_loader` 로 로더를 분리해 재기 전까지는 이 가정을 의심하지 못했다.

---

### 남은 IO 개선은 헛수고였나

아니다. 로더가 6,661 seg/s 수준이었다면 동기화를 없애도 거기서 막혔을 것이다.
다만 **순서가 비효율적이었다.** 로더를 먼저 분리 측정했다면 세 번의 IO 수정 중 일부는
나중으로 미룰 수 있었다. 아래 "다음에 느릴 때 볼 순서" 는 그 교훈을 반영한 것이다.
